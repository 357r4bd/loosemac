...
my %reverse_msg_lookup = ('-1' => 'CORRUPTMSG',     1  => 'BEACONMSG',
		            2  => 'CONFLICTREPORT', 4  => 'BEACON_CONFLICT',);
...
my %reverse_transition_lookup = ( A => 'HEARDBEACON',       B => 'SENTMSG', C => 'HEARDCONFLICT',
                                  D => 'DETECTEDCOLLISION', E => 'DETECTEDCOLLISION',);
#
...
sub get_machine { 
  my %machine = (
    0 => { A => \&heard_beacon , B => \&sent_msg, C => \&nothing,        D => \&detected_collision_noreset, E => \&nothing,    }, 
    1 => { A => \&heard_beacon , B => \&nothing , C => \&heard_conflict, D => \&detected_collision_reset,   E => \&make_ready, }, 
...
sub sent_msg {
  ...
  print  "  $node sent BEACONMESG to: [" . join(',',@{$adj_ref}) . "] and will be ready @ next slot $slot (t=$readytimestep)\n";
...
sub get_status_all () {
  ...
  foreach (sort {$a <=> $b} keys(%graph)) {
    ...
    $out .= "  Node $_ $slot is <".$reverse_state_lookup{$current_states{$_}} . 
            "> Adjacent to [" . join(',',@{$graph{$_}}) . 
